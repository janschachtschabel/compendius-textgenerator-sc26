...
| `POST /api/v2/qa` | Frage-Antwort-Paare zu `text` oder `topic` (dann aus den Artikeln des Themas, 404 mit `resolution`, wenn es sie nicht gibt). `method` wählt `rule-based` (Fragevorlagen über die Sätze, braucht nichts, Standard) oder `llm` (die b-api schreibt sie); ohne nutzbare b-api oder ohne verwertbare Antwort fällt es auf `rule-based` zurück und `note` sagt warum. `count` und `max_answer_length` begrenzen |
