...
| M32 Lehrplanbezüge nach D58 | `mc_lehrplan_pruefung.py rules <out.json>` stellt die 40 Anfragen von M22 mit den Regeln, `mc_lehrplan_pruefung.py check <out.json>` mit `curriculum_check=llm` (eine Wanze auf `check_curriculum` hält jede Note fest), `mc_lehrplan_pruefung.py m2v <modell> <out.json>` rechnet die Ähnlichkeit für die Stichprobe von M22 (D1), `mc_lehrplan_pruefung.py evaluate <regeln.json> <prüfung.json> <m2v.json> <out.json>` wertet mit den Noten von M22 aus; alle aus dem Projektordner, mit `lehrplan.db` in `STATE_DIR` und den Archiven von kompendium-test | venv dieses Projekts | `gpt-6-luna` für `check`, 592.835 Tokens |
| M33 Budget von `best-quality` nach D59 | `mc_budget_best_quality.py <out.json> <budget> [--search]` stellt Demokratie ohne Fach als Kompendium mit Teil 1 und 2 in beiden `best-quality`-Profilen und mit `--search` als Lehrplansuche mit `mode=topic`, über die API im Prozess, mit dem Budget der Befehlszeile; jeder Lauf kommt in dieselbe Datei; aus dem Projektordner, mit `lehrplan.db` in `STATE_DIR` und den Archiven von kompendium-test | venv dieses Projekts | `gpt-6-luna`, drei Läufe (120.000 mit `--search`, 200.000, 180.000) mit 187.642, 289.595 und 289.595 Tokens gemeldet; was die b-api schon kannte, kam aus ihrem Zwischenspeicher |
| M34 QA-Regeln nach D60 | `mc_qa_nachschaerfung.py pairs <m30_free.json> <out.json>` im Image, einmal mit dem installierten Code und einmal mit `PYTHONPATH` auf eine Kopie der Arbeitsstände (je 20 Paare auf den Texten von M30), `mc_qa_nachschaerfung.py sheets <m30-bögen> <m30_qa.json> <paare.json> <ordner>` setzt die neuen Paare in die Bögen von M30, `mc_qa_nachschaerfung.py evaluate …` zählt mit den Urteilen beider Gutachter | Image für `pairs`, sonst beliebiges Python | keins; zwei Claude-Gutachter |
| M35 Sichere Auflösungen mehrdeutiger Wörter | `mc_sichere_aufloesung.py <out.json> [--dry] <gold.yaml>...` stellt jede Anfrage der drei Gold-Dateien von `eval/artikelwahl` durch `choose_main_article`: Regeln, LLM für unsichere Fälle, A dazu sichere Begriffsklärungen, B dazu exakte Titel mit Begriffsklärungsseite; `--dry` zählt die Aufrufe, ohne das Modell zu fragen; aus dem Projektordner, mit den Archiven von kompendium-test | venv dieses Projekts | `gpt-6-luna`, Staging-b-api; zwei Läufe (vor und nach `20aaca4`), 60.169 Tokens gemeldet im zweiten, davon 42.462 für die 46 neuen Fragen |
...
| `ergebnisse/m32_lehrplan_pruefung.json` | M32, die Anteile je Variante (alles, B, LLM-Prüfung) und Gutachter, je Anfrage Elemente, Gebündelte, Verworfene, Tokens, Sekunden und Rückfälle, je Element der Stichprobe von M22 Fundort, Note des LLM, beide Noten und die Ähnlichkeit; keine Texte |
| `ergebnisse/m33_budget_best_quality.json` | M33, je Lauf das Budget und je Weg (Kompendium je Profil, Lehrplansuche) Status, Sekunden, Tokens und Aufrufe, die Zuordnung (Absätze, vom LLM entschieden) und die Prüfung (Elemente, geprüft, verworfen, Rückfälle mit Grund); keine Texte |
| `ergebnisse/m34_qa_regeln.json` | M34, die Paare der Regeln vor und nach D60 je Herkunft (Text, Glossar, Füller, Wiederholung) mit ihren Urteilen, die Mängel, die Gegenprobe an den anderen Verfahren, das Verfahren je Kennung und die Urteile beider Gutachter; keine Texte und Paare |
| `ergebnisse/m35_sichere_aufloesung.json` | M35, je Anfrage und Variante Titel, Methode, sicher, Begriffsklärung, gefragt, Kandidaten, Tokens, Modell und Sekunden, die Zusammenfassung je Variante und der erste Lauf vor `20aaca4` mit seinen Zeiten; keine Artikeltexte |
