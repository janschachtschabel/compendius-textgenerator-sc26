...
Stand 23.09.2026 · neuer Dienst v2.0.0 (`compendious-text-fastapi`, GitHub `compendius-textgenerator-sc26`) ·
alter Dienst v0.2.0 (`alterCode/compendious`) · der LLM-Zuordner `matcher=llm` (D34) kam nach v2.0.0 hinzu: Er steht
auf `main` und im Image `:latest`, eine Version mit Tag gibt es dafür noch nicht

Diese Seiten beschreiben, wie der Kompendium-Dienst für das Sommercamp 2026 (SC26) neu gebaut wurde, was vom alten
Dienst geblieben ist und warum die Verfahren so gewählt sind. Die Messungen vom 23.09.2026 stehen mit Aufbau und
...
- **Schneller, ohne Tokens.** Weltwissen und Lehrplanbezüge brauchen auf dem Server im Median 2,8 s und kein
  Sprachmodell. Der alte Dienst brauchte im besten Fall 35 s und rund 7.900 Tokens. So wie er ausgeliefert ist,
  weist Wikipedia seine Anfragen ab: Er lief dann 374 s und lieferte einen Text ohne eine einzige Quelle, aber mit
  Belegnummern.
...
- **Artikelwahl gemessen.** Den Hauptartikel trifft der Dienst bei normalen Themen und Klassenzusätzen immer (28
  von 28), bei mehrdeutigen Wörtern mit Fachangabe nur in 6 von 16 Fällen; die Ursachen liegen in der Wertung der
  Kontextwörter und sind behebbar. Auf den zehn Goldthemen passen 6 % der Korpusartikel nicht zum Thema, beim alten
  Dienst 14 %. Die schwächste Quelle sind die Volltexttreffer je Baustein.
- **Zuordnung zu den zehn Inhaltsbausteinen des SC26-Templates** über eine Regel-Policy mit einfachen Rankern
  ...
  schlechter ab. Ein LLM als Zuordner (`gpt-5.6-luna`) ist deutlich besser, 0,66 statt 0,43 auf den gelabelten
  Absätzen, und ist seit D34 als `matcher=llm` wählbar; es kostet rund 240 Tokens je Absatz.
...
| Artikelwahl | LLM rät zehn Artikeltitel | Titel, Weiterleitung, Begriffsklärung, Titelvorschläge, Volltextsuche |
...
| Dauer | 35 s (bester Fall) bis 374 s (Wikipedia weist ab) | 2,8 s für Teil 1 und 2 (Median, Server) |
| Tokens je Kompendium | rund 7.900 | 0 im Standard; mit LLM-Schaltern 2.300 bis rund 37.000, mit `matcher=llm` im Median rund 39.000 mehr |
...
5. [Messprotokoll](05-messprotokoll.md): Aufbau, Zahlen, Skripte

Die Seiten sind als Baum für Confluence gedacht: diese Übersicht als Elternseite, die fünf übrigen darunter. Die
